...
def latest_builds():
    response = http.request(
        "GET", circleci_build_url, headers={"Accept": "application/json"}
    )
    ...
    list = zip(
        ["32bit", "64bit", "appimage-64bit", "wasm", "macos"],
        [builds32, builds64, appimages64, wasm, macos],
    )
...
def latest_artifacts(builds):
    ...
    for build in builds:
        response = http.request(
            "GET",
            f"{circleci_base_url}/{build}/artifacts",
...
def new_builds():
    ...
    for key in ["32bit", "64bit", "appimage-64bit", "wasm", "macos"]:
        if key not in last_builds or (
            key in builds and last_builds[key] != builds[key]
        ):
            if key in builds:
