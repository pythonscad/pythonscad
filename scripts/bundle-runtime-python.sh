...
#
#   1. Packages from pyproject.toml [dependency-groups] bundle (via uv).
...
# Optional: --python <interpreter>   target Python (default: python3)
#           --no-licenses            skip THIRD_PARTY_LICENSES.txt step
#
# Exit codes:
#   0  success
#   1  unrecoverable error (bad arguments, uv install failed, ...)
...
info "  project:      ${PROJECT_ROOT} (dependency group: bundle)"
...
# from a previous run: `uv pip install --target ... --upgrade` only
# add/upgrades packages, it does NOT prune packages that used to be in
# the bundle group but are not anymore. Re-running into ${DEST}
...
if ! install_bundle_deps; then
    die "runtime dependency install into ${STAGING_DEST} failed"
fi
