...
def find_dependencies(file):
    ...
    for dep in deps:
        ...
        if (
            len(dep) > 0
            and not re.search("/System/Library", dep)
            and not re.search("/usr/lib", dep)
        ):
            libs.append(dep)
    return libs
...
def validate_lib(lib):
    p = subprocess.Popen(
        ["otool", "-l", lib], stdout=subprocess.PIPE, universal_newlines=True
    )
    ...
    if deploymenttarget is None:
        print(
            "Error: Neither LC_VERSION_MIN_MACOSX nor LC_BUILD_VERSION found in " + lib
        )
        return False
...
if __name__ == "__main__":
    ...
    p = subprocess.Popen(
        ["otool", "-l", executable], stdout=subprocess.PIPE, universal_newlines=True
    )
    output = p.communicate()[0]
    if p.returncode != 0:
    ...
    lc_rpath = m.group(2)
    if DEBUG:
        print("Runpath search path: " + lc_rpath)
    ...
    while len(pending) > 0:
        ...
        for d in deps:
            ...
            is_homebrew_dep = absfile.startswith(
                "/usr/local/opt/"
            ) or absfile.startswith("/opt/homebrew/")
            if not absfile.startswith(executable_path) and not (
                args.allow_homebrew_deps and is_homebrew_dep
            ):
            ...
            if args.allow_homebrew_deps and is_homebrew_dep:
                if DEBUG:
                    print("Skipping validation of Homebrew dependency: " + absfile)
                continue
