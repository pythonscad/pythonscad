...
get_arch_deps()
{
  ...
  base-devel boost cairo cgal cmake double-conversion eigen fontconfig freetype2 \
  gcc-libs ghostscript glew glib2 glibc glu gmp harfbuzz hicolor-icon-theme hidapi \
  imagemagick lib3mf libglvnd libspnav libx11 libxml2 libzip mimalloc mpfr nettle \
  opencsg python python-pip python-setuptools procps-ng qscintilla-qt5 \
  qt5-base qt5-multimedia qt5-svg tbb xorg-server-xvfb
}
