...
int python_more_obj(std::vector<std::shared_ptr<AbstractNode>>& children, PyObject *more_obj)
{
  ...
  } else if (pf.PyObject_IsInstance(more_obj, reinterpret_cast<PyObject *>(&PyOpenSCADType))) {
...
{
  if (pf.PyObject_IsInstance(objs, reinterpret_cast<PyObject *>(&PyOpenSCADType))) {
    return Py_TYPE(objs);
...
int python_numberval(PyObject *number, double *result, int *flags, int flagor)
{
  ...
  if (PyNumber_CHECK(number)) {
    ...
    PyObject *f = pf.PyNumber_Float(number);
    *result = pf.PyFloat_AsDouble(f);
    return 0;
  }
    ...
    if (pf.PyList_Size(vec) >= mindim && pf.PyList_Size(vec) <= maxdim) {
      for (int i = 0; i < pf.PyList_Size(vec); i++) {
        if (pf.PyList_Size(vec) > i) {
          if (python_numberval(pf.PyList_GetItem(vec, i), &result[i], dragflags, 1 << i))
...
double python_doublefunc(void *v_cbfunc, double arg)
{
  ...
  Py_XDECREF_(args);
  if (funcresult) result = pf.PyFloat_AsDouble(funcresult);
  return result;
}
...
PyObject *python_fromopenscad(const Value& val)
{
  ...
  case Value::Type::NUMBER:    return pf.PyFloat_FromDouble(val.toDouble());
  case Value::Type::STRING:    return pf.PyUnicode_FromString(val.toString().c_str());
  case Value::Type::VECTOR:    {
    const VectorType& vec = val.toVector();
    PyObject *result = pf.PyList_New(vec.size());
    for (size_t j = 0; j < vec.size(); j++) pf.PyList_SetItem(result, j, python_fromopenscad(vec[j]));
    return result;
  }
...
void python_catch_error(std::string& errorstr)
{
  ...
  if (pyExcValue != nullptr) {
    ...
    PyObjectUniquePtr pyExcValueStr(pf.PyUnicode_AsEncodedString(str_exc_value.get(), "utf-8", "~"),
                                    PyObjectDeleter);
...
Value python_convertresult(PyObject *arg, int& error)
{
  ...
  if (PyList_CHECK(arg)) {
    ...
    return {pf.PyFloat_AsDouble(arg)};
  } else if (arg == Py_FALSE) {
    return false;
  } else if (arg == Py_TRUE) {
    return true;
  } else if (PyLong_CHECK(arg)) {
    return {(double)pf.PyLong_AsLong(arg)};
...
void initPython(const std::string& binDir, const std::string& scriptpath, double time)
{
  ...
  std::ostringstream stream;
  stream << "t=" << time << "\nphi=" << 2 * G_PI * time;
  pf.PyRun_StringFlags(stream.str().c_str(), Py_file_input, pythonInitDict.get(), pythonInitDict.get(),
                       nullptr);
...
";
  ...
  result.reset(pf.PyRun_String(code.c_str(), Py_file_input, pythonInitDict.get(),
  ...
  for (int i = 0; i < 2; i++) {
    ...
    catcher.reset(
      pf.PyObject_GetAttrString(pythonMainModule.get(), i == 1 ? "catcher_err" : "catcher_out"));
...
extern "C" PyObject *PyInit_openscad(void) { return pf.PyModule_Create(&OpenSCADModule); }
