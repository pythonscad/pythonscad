...
private:
  ...
  std::vector<std::shared_ptr<const HyperObject>> collectChildren4D(const AbstractNode& node);
  ...
  std::unique_ptr<HyperObject> applyToChildren4D(const AbstractNode& node, OpenSCADOperator op);
