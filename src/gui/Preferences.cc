    ...
    for (const auto& p : profiles) {
      ...
      if (p == _("Ollama Local")) {
        ...
        params["system_prompt"] =
          ...
          "   - NEVER output OpenSCAD code in markdown block format. ALWAYS use the `set_editor_code` "
          "tool so the user can apply it.\n"
...
void Preferences::loadAIParams(const QString& profileName)
{
  ...
  if (sysPrompt.empty()) {
    sysPrompt =
      ...
      "3. **Tool Workflow**:\n"
      "   - YOU MUST USE `set_editor_code` TO PROPOSE ANY CODE CHANGES.\n"
      "   - NEVER output OpenSCAD code in markdown block format. ALWAYS use the `set_editor_code` tool "
