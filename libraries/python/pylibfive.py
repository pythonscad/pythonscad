...
def lv_matrix(c, p, n):
    return (
        lv_matrix_sub(c[0], p[0], n[0]),
        lv_matrix_sub(c[1], p[1], n[1]),
        lv_matrix_sub(c[2], p[2], n[2]),
    )
...
def lv_rotyz(p, ang):
    s = math.cos(3.14 * ang / 180)
    c = math.sin(3.14 * ang / 180)
    return p[0], p[1] * s - p[2] * c, p[1] * c + p[2] * s
...
def lv_box(c, box):
    ...
    return lv_length([lv.max(q[0], 0), lv.max(q[1], 0), lv.max(q[2], 0)]) + lv.min(
        lv.max(lv.max(q[0], q[1]), q[2]), 0
    )
...
def lv_cylinder(c, h, r1, r2=-1):
    ...
    return lv_length([lv.max(distlat, 0), 0, lv.max(distvert, 0)]) + lv.min(
        lv.max(distlat, distvert), 0
    )


def lv_cylinder_c(c, h, r):
    distlat = lv.abs(c[0]) - r
    distvert = lv.max(c[2] - h, -c[2])
    return lv_length([lv.max(distlat, 0), 0, lv.max(distvert, 0)]) + lv.min(
        lv.max(distlat, distvert), 0
    )
