...
class LaserCutter:
    ...
    def link(self):
        ...
        for i,f in enumerate(self.faces):
            ...
            self.faces[i] = pol.multmatrix(f.matrix)
        ...
        faceprocess = self.faces
        self.faces = []
        for i, f in enumerate(faceprocess):
            mat = f.matrix
            pol, = f.children()
            if len(pol.paths) == 0:
                pol.paths=[list(range(len(pol.points)))]
            ...
            for j, path in enumerate(pol.paths):
                ...
                for k in range(n): # all edges
                    ...
                    if conn == conn_plain:
                        ...
                        for l, oface in enumerate(faceprocess):
                            if l == i:
                                continue
                            # pos auf oface
                ...
                for k, oface in enumerate(faceprocess):
                    if k == i:
                        continue
                    oshape,  = oface.children()
        ...
        for i, piece in enumerate(self.faces):
            ...
            newfaces.append(multmatrix(newpol, piece.matrix))
        self.faces = newfaces
...
################
# Fixme: need to review integration and if ths should be pulled into
# the Laser class above.
...
# colorstr - return the working labled color as a string
# representation of the hex value starting with a '#'
def colorstr(key):
    return "#{:X}".format(int(Color_Table[key]["color"]))
